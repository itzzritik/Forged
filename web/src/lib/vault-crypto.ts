...
type WorkerMessage =
  | { type: "hash"; masterPasswordHash: Uint8Array }
  | { type: "key"; cryptoKey: CryptoKey }
...
function getWorker(): Worker {
  if (!activeWorker) {
    activeWorker = new Worker(
      new URL("./vault-crypto-worker.ts", import.meta.url),
    );
  }
  return activeWorker;
}
...
export async function decryptBlob(
  ...
  const keys: VaultKeyMetadata[] = (parsed.keys || []).map((k: any) => ({
    ...
    gitSigning: k.git_signing || false,
  }));
...
export async function decryptItemKey(
  symmetricKey: CryptoKey,
  encryptedCipherKeyB64: string,
): Promise<CryptoKey> {
  const data = Uint8Array.from(atob(encryptedCipherKeyB64), (c) =>
    c.charCodeAt(0),
  );
...
export async function decryptPrivateKey(
  ...
  const data = Uint8Array.from(atob(encryptedPrivateKeyB64), (c) =>
    c.charCodeAt(0),
  );
  const nonce = data.slice(0, 12);
  const ciphertext = data.slice(12);
