...
const (
	...
	HeaderSize            = 8 + 2 + SaltSize + 4 + 4 + 1 + NonceSize // 75 bytes
)
...
func ReadHeader(r io.Reader) (Header, error) {
	...
	if h.Version > CurrentVersion {
		return Header{}, fmt.Errorf("vault version %d is newer than supported (%d), please upgrade forged", h.Version, CurrentVersion)
	}
